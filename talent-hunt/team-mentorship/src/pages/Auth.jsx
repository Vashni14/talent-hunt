...
function Auth() {
  ...
  const register = async () => {
    try {
      ...
      navigate(`/login`); // Navigate based on selected role
    } catch (error) {
  ...
  const login = async () => {
    try {
      ...
      navigate(userRole === "student" ? "/student/dashboard" : userRole === "mentor" ? "/mentor-dashboard" : "/admin-dashboard");
    } catch (error) {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      // Create user document if new user (pseudo-code)
