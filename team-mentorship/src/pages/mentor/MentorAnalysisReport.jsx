...
const MentorAnalysisReport = () => {
 ...
 const fetchData = async () => {
    try {
      ...
      try {
        const profileRes = await axios.get(`http://localhost:5000/api/mentor/profile/${userId}`);
      ...
      try {
        if (data.teams.length > 0) {
          const compRes = await axios.get(`http://localhost:5000/api/compapp/mentor/${data.profile._id}`);
