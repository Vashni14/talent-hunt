...
const MentorProfile = () => {
  ...
  useEffect(() => {
    const fetchMentorProfile = async (userId) => {
      try {
        const response = await axios.get(`https://talent-hunt-2.onrender.com/api/mentor/profile/${userId}`);
        ...
        if (mentorData) {
          ...
          if (mentorData.profilePicture) {
            setImagePreview(`https://talent-hunt-2.onrender.com${mentorData.profilePicture}`);
          }
        }
      } catch (error) {
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      if (profileImage) {
        ...
        setImagePreview(`https://talent-hunt-2.onrender.com${updatedMentor.profilePicture}`);
      }
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
              <div className="relative group">
                <div className="w-24 h-24 rounded-full bg-gray-700 overflow-hidden border-4 border-blue-500/30 flex-shrink-0">
                  <img 
                    src={imagePreview || (user?.profilePicture ? `https://talent-hunt-2.onrender.com${user.profilePicture}` : "/default-profile.png")}
                    alt="Profile" 
                    className="w-full h-full object-cover"
                    onError={(e) => {
                      e.target.onerror = null;
