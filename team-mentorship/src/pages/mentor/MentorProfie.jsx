...
const MentorProfile = () => {
  ...
  useEffect(() => {
    const fetchMentorProfile = async (userId) => {
      try {
        const response = await axios.get(`https://team-match.up.railway.app/api/mentor/profile/${userId}`);
        ...
        if (mentorData) {
          ...
          if (mentorData.profilePicture) {
            setImagePreview(`https://team-match.up.railway.app${mentorData.profilePicture}`);
          }
        }
      } catch (error) {
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      const response = await axios.put(
        `https://team-match.up.railway.app/api/mentor/profile/${userId}`,
        formDataToSend,
      );
      ...
      if (profileImage) {
        ...
        setImagePreview(`https://team-match.up.railway.app${updatedMentor.profilePicture}`);
      }
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
              <div className="relative group">
                <div className="w-24 h-24 rounded-full bg-gray-700 overflow-hidden border-4 border-blue-500/30 flex-shrink-0">
                  <img 
                    src={imagePreview || (user?.profilePicture ? `https://team-match.up.railway.app${user.profilePicture}` : "/default-profile.png")}
                    alt="Profile" 
                    className="w-full h-full object-cover"
                    onError={(e) => {
                      e.target.onerror = null;
