  ...
  useEffect(() => {
    ...
    const fetchStudentProfile = async () => {
      try {
        ...
        const response = await axios.get(`https://team-match.up.railway.app/api/student/profile/${userId}`);
        setCurrentUser({
  ...
  const fetchMyOpenings = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://team-match.up.railway.app/api/invitations/openings/user/${userId}`,
        {
          headers: {
...
const fetchSentApplications = useCallback(async () => {
  try {
    ...
    const response = await axios.get(
      `https://team-match.up.railway.app/api/invitations/applications/sent/${userId}`, 
      {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Cache-Control': 'no-cache'
      ...
      response.data.map(async app => {
        try {
          const creatorResponse = await axios.get(
            `https://team-match.up.railway.app/api/student/profile/${app.opening.createdBy}`,
            {
              headers: {
                'Authorization': `Bearer ${token}`
              }
...
const fetchReceivedApplications = useCallback(async () => {
  try {
    ...
    const response = await axios.get(
      `https://team-match.up.railway.app/api/invitations/applications/received/${userId}`, 
      {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Cache-Control': 'no-cache'
...
const fetchOpenings = useCallback(async () => {
  ...
  try {
    ...
    const response = await axios.get('https://team-match.up.railway.app/api/invitations/openings', { params });
    ...
    const filteredOpenings = Array.isArray(response?.data) 
      ? await Promise.all(
          response.data
            ...
            .map(async opening => {
              try {
                const creatorResponse = await axios.get(`https://team-match.up.railway.app/api/student/profile/${opening.createdBy}`);
                return {
                  ...opening,
                  creator: {
                    name: creatorResponse.data.name,
  ...
  const saveTeamOpening = async () => {
    try {
      ...
      const endpoint = selectedOpening 
        ? `https://team-match.up.railway.app/api/invitations/openings/${selectedOpening._id}`
        : 'https://team-match.up.railway.app/api/invitations/teams/openings';
  ...
  const deleteTeamOpening = async (openingId) => {
    try {
      ...
      await axios.delete(`https://team-match.up.railway.app/api/invitations/openings/${openingId}`);
  ...
  const applyToOpening = async (openingId, message) => {
    try {
      ...
      const response = await axios.post(
        `https://team-match.up.railway.app/api/invitations/openings/${openingId}/apply`, // Fixed endpoint
        { 
          message,
          userId
        },
  ...
  const updateApplicationStatus = async (applicationId, status) => {
    try {
      ...
      const response = await axios.put(`https://team-match.up.railway.app/api/invitations/applications/${applicationId}`, { status },
     {
             headers: {
               'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const withdrawApplication = async (applicationId) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `https://team-match.up.railway.app/api/invitations/applications/${applicationId}/withdraw`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handleViewProfile = async (applicant) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://team-match.up.railway.app/api/student/profile/${userId}`,
      );
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-gray-700/50 rounded-lg p-4 border border-gray-600">
                      ...
                      <div className="flex items-center gap-2">
                        <img
                          src={selectedOpening.creator?.profilePicture ? `https://team-match.up.railway.app${selectedOpening.creator.profilePicture}` : "/default-profile.png"}
