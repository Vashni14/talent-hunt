    ...
    for (const team of myTeams) {
      try {
        const appsResponse = await axios.get(
          `https://team-match.up.railway.app/api/mentor/applications/team/${team._id}`
        );
...
useEffect(() => {
  ...
  const handleViewProfile = async (mentor) => {
    ...
    try {
      const response = await axios.get(`https://team-match.up.railway.app/api/mentor/profile/${mentor.userId}`);
  ...
  const handleSendRequest = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://team-match.up.railway.app/api/mentor/applications', {
  ...
  const handleWithdraw = async (applicationId) => {
    try {
      const response = await axios.delete(`https://team-match.up.railway.app/api/mentor/applications/${applicationId}`);
    ...
    <img
      src={
        app.mentor?.profilePicture
          ? `https://team-match.up.railway.app${app.mentor.profilePicture}`
                ...
                <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 mb-6">
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                    <div className="w-24 h-24 rounded-full bg-gray-700 overflow-hidden border-4 border-blue-500/30 flex-shrink-0">
                      <img 
                        src={mentorDetails?.profilePicture ? `https://team-match.up.railway.app${mentorDetails.profilePicture}` : "/default-profile.png"}
                        alt="Profile" 
                        className="w-full h-full object-cover"
                        onError={(e) => {
                          e.target.onerror = null;
