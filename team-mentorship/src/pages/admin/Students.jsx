...
const CompetitionParticipationModal = ({ userId, onClose }) => {
  ...
  const fetchParticipationData = async () => {
    try {
      ...
      const studentResponse = await axios.get(`https://team-match.up.railway.app/api/student/profile/${userId}`);
      if (!studentResponse.data) {
        throw new Error('User not found');
      }

      // Get all applications for this student
      const applicationsResponse = await axios.get(`https://team-match.up.railway.app/api/compapp/me/${userId}`);
...
const MyTeamsModal = ({ userId, onClose }) => {
  ...
  const fetchMentorDetails = async (mentorId) => {
    try {
      ...
      const response = await axios.get(`https://team-match.up.railway.app/api/mentor/profile/id/${mentorId}`);
      if (response.data) {
  ...
  const fetchTeams = async () => {
    try {
      ...
      const response = await axios.get(`https://team-match.up.railway.app/api/teams/by-user/${userId}`);
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            <img
                              ...
                              src={team.createdBy?.profilePicture ? `https://team-match.up.railway.app${team.createdBy.profilePicture}` : "/default-profile.png"}
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get('https://team-match.up.railway.app/api/student/profile');
  ...
  const handleDeleteStudent = async (studentId) => {
    ...
    try {
      setLoading(true);
      const response = await axios.delete(`https://team-match.up.railway.app/api/student/${studentId}`, {
        headers: {
  ...
  const searchStudents = async (username) => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get(`https://team-match.up.railway.app/api/student/profile/username/${username}`);
  ...
  const fetchStudentByUid = async (uid) => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get(`https://team-match.up.railway.app/api/student/profile/${uid}`);
                ...
                <div className="flex items-center">
                  <div className="flex-shrink-0 h-10 w-10">
                    <img
                      ...
                      src={student?.profilePicture ? `https://team-match.up.railway.app${student.profilePicture}` : "/default-profile.png"}
                      alt={student.name}
                    />
                  </div>
