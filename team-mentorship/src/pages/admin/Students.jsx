              ...
              <div className="md:col-span-1">
                <div className="flex flex-col items-center">
                  <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-blue-500/50 mb-4">
                    <img
                      src={profile?.profilePicture ? `https://talent-hunt-3.onrender.com${profile.profilePicture}` : "/default-profile.png"}
                      alt={profile.name}
...
const CompetitionParticipationModal = ({ userId, onClose }) => {
  ...
  const fetchParticipationData = async () => {
    try {
      ...
      const studentResponse = await axios.get(`https://talent-hunt-3.onrender.com/api/student/profile/${userId}`);
      if (!studentResponse.data) {
        throw new Error('User not found');
      }

      // Get all applications for this student
      const applicationsResponse = await axios.get(`https://talent-hunt-3.onrender.com/api/compapp/me/${userId}`);
...
const MyTeamsModal = ({ userId, onClose }) => {
  ...
  const fetchMentorDetails = async (mentorId) => {
    try {
      ...
      const response = await axios.get(`https://talent-hunt-3.onrender.com/api/mentor/profile/id/${mentorId}`);
      if (response.data) {
  ...
  const fetchTeams = async () => {
    try {
      ...
      const response = await axios.get(`https://talent-hunt-3.onrender.com/api/teams/by-user/${userId}`);
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            <img
                              ...
                              src={team.createdBy?.profilePicture ? `https://talent-hunt-3.onrender.com${team.createdBy.profilePicture}` : "/default-profile.png"}
                      ...
                      {team.members?.map((member, idx) => (
                        <div key={idx} className="flex items-center justify-between">
                          <div className="flex items-center">
                            <img
                              className="w-6 h-6 rounded-full mr-2"
                              src={member.user?.profilePicture ? `https://talent-hunt-3.onrender.com${member.user.profilePicture}` : "/default-profile.png"}
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get('https://talent-hunt-3.onrender.com/api/student/profile');
  ...
  const handleDeleteStudent = async (studentId) => {
    ...
    try {
      setLoading(true);
      const response = await axios.delete(`https://talent-hunt-3.onrender.com/api/student/${studentId}`, {
        headers: {
  ...
  const searchStudents = async (username) => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get(`https://talent-hunt-3.onrender.com/api/student/profile/username/${username}`);
  ...
  const fetchStudentByUid = async (uid) => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get(`https://talent-hunt-3.onrender.com/api/student/profile/${uid}`);
                ...
                <div className="flex items-center">
                  <div className="flex-shrink-0 h-10 w-10">
                    <img
                      ...
                      src={student?.profilePicture ? `https://talent-hunt-3.onrender.com${student.profilePicture}` : "/default-profile.png"}
                      alt={student.name}
                    />
                  </div>
