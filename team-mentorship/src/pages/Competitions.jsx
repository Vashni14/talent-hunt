...
useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [competitionsRes, applicationsRes, teamsRes] = await Promise.all([
          axios.get('https://talent-hunt-3.onrender.com/api/competitions'),
          axios.get(`https://talent-hunt-3.onrender.com/api/compapp/me/${userId}`),
          axios.get(`https://talent-hunt-3.onrender.com/api/teams/user/${userId}`)
        ]);
  ...
  const submitApplication = async () => {
    try {
      ...
      const response = await axios.post(
        `https://talent-hunt-3.onrender.com/api/compapp/${currentCompetition._id}/apply/${userId}`,
        {
  ...
  const updateCompetitionResult = async (applicationId, result, analysis) => {
    try {
      const token = await auth.currentUser.getIdToken();
      await axios.put(
        `https://talent-hunt-3.onrender.com/api/compapp/${applicationId}/result/${userId}`,
        { result, analysis },
        { headers: { Authorization: `Bearer ${token}` } }
      );
        ...
        <div className="bg-gray-800 rounded-xl w-full max-w-4xl border border-gray-700 overflow-hidden shadow-xl">
          ...
          <div className="overflow-y-auto max-h-[80vh]">
            <div className="h-64 w-full bg-gray-700 overflow-hidden relative">
              <img 
                src={currentCompetition?.photo ? `https://talent-hunt-3.onrender.com${currentCompetition.photo}` : "/default-profile.png"}
                ...
                return (
                  <div key={competition._id} className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 hover:border-blue-500/30 hover:shadow-lg hover:shadow-blue-500/10 transition-all duration-300">
                    <div className="h-40 overflow-hidden relative">
                      <img 
                        src={competition?.photo ? `https://talent-hunt-3.onrender.com${competition.photo}` : "/default-profile.png"}
