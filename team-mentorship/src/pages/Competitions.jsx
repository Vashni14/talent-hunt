...
const StudentCompetitions = () => {
  ...
  const submitApplication = async () => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:5001/api/competitions/${currentCompetition._id}/apply`,
        applicationForm
      );
  ...
  const ApplicationModal = () => {
    ...
    const [isLoadingTeams, setIsLoadingTeams] = useState(false);
      ...
      // Fetch user's teams when modal opens AND we have a user ID
 
      const [isLoading, setIsLoading] = useState(false);
      const [error, setError] = useState(null);
      
      // Fetch user's teams - now memoized and only runs when needed
      const fetchMyTeams = useCallback(async () => {
        ...
        try {
          ...
          const teamsData = Array.isArray(responseData) ? responseData : 
                           (responseData.data || []);
      ...
      }, [isApplicationModalOpen, fetchMyTeams]); // Stable fetchMyTeams reference
    ...
    const submitApplication = async () => {
      try {
        const applicationData = {
          ...
          teamId: selectedTeam // Changed from teamMembers to teamId
        };
        ...
        <div className="bg-gray-800 rounded-xl w-full max-w-2xl border border-gray-700 overflow-hidden shadow-xl">
          ...
          <div className="p-6 overflow-y-auto max-h-[70vh]">
            <div className="space-y-4">
              ...
              <div>
                ...
                {myTeams.length === 0 && !isLoadingTeams && (
          ...
          <div className="bg-gray-900 px-6 py-4 border-t border-gray-700 flex justify-end space-x-3">
            ...
            <button
              ...
              disabled={!applicationForm.motivation || !applicationForm.skills || !selectedTeam}
              className={`px-4 py-2 rounded-lg transition-colors ${
                (!applicationForm.motivation || !applicationForm.skills || !selectedTeam)
            ...
            <div key={competition._id} className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 hover:border-blue-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/10">
              <div className="h-40 overflow-hidden relative">
                <img 
                  ...
                  onError={(e) => {
                    e.target.onerror = null; 
                    e.target.src = 'https://via.placeholder.com/500x300?text=Competition';
                  }}
                />
