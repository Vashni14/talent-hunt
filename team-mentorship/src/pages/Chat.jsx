      ...
      try {
        if (activeTab === "individual") {
          const res = await axios.get(
            `http://localhost:5000/api/chat/conversations/${currentUser.uid}`
          );
          setIndividualChats(res.data);
        } else if (activeTab === "team") {
          const res = await axios.get(
            `http://localhost:5000/api/chat/team-conversations/${currentUser.uid}`
          );
          setTeamChats(res.data);
        } else if (activeTab === "mentor") {
          if (currentUser.role === "mentor") {
            const res = await axios.get(
              `http://localhost:5000/api/chat/mentor-students/${currentUser.uid}`
            );
            setMentorChats(res.data);
          } else {
            const res = await axios.get(
              `http://localhost:5000/api/chat/student-mentors/${currentUser.uid}`
            );
            setMentorChats(res.data);
          }
        }
  ...
  useEffect(() => {
    ...
    const searchTimeout = setTimeout(async () => {
      try {
        setIsSearching(true);
        const res = await axios.get(
          `http://localhost:5000/api/chat/search/${currentUser.uid}/${searchTerm}`
        );
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        if (activeTab === "individual") {
          res = await axios.get(
            `http://localhost:5000/api/chat/messages/${currentUser.uid}/${activeChat}`
          );
        } else if (activeTab === "team") {
          res = await axios.get(
            `http://localhost:5000/api/chat/team-messages/${activeChat}?userId=${currentUser.uid}`
          );
        } else if (activeTab === "mentor") {
          res = await axios.get(
            `http://localhost:5000/api/chat/messages/${currentUser.uid}/${activeChat}`
