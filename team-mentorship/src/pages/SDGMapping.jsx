...
export default function SDGMapping() {
  ...
  useEffect(() => {
    if (showDashboardModal) {
      const fetchStats = async () => {
        try {
          const response = await fetch('https://talent-hunt-2.onrender.com/api/sdgs/stats');
  ...
  const handleMapSDGs = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://talent-hunt-2.onrender.com/api/sdgs/team/${selectedTeamForMapping._id}`, {
        method: 'PUT',
        headers: {
