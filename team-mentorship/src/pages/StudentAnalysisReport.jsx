...
const StudentAnalysisReport = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
     ...
     // Fetch profile data
try {
    const profileRes = await axios.get(`https://team-match.up.railway.app/api/student/profile/${userId}`);
  ...
  let userTeams = [];
  try {
    const teamsRes = await axios.get(`https://team-match.up.railway.app/api/teams/user/${userId}`);
  ...
  if (userTeams.length > 0) {
    try {
      ...
      if (teamIds.length > 0) {
        ...
        const mentorsRes = await axios.post(`https://team-match.up.railway.app/api/teams`, { teamIds });
  ...
  try {
    const compAppsRes = await axios.get(`https://team-match.up.railway.app/api/compapp/me/${userId}`);
  ...
  // Fetch goals
  try {
    const goalsRes = await axios.get(`https://team-match.up.railway.app/api/goals/${userId}`);
