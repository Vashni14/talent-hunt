...
const [deadlinesLoading, setDeadlinesLoading] = useState(true);
  ...
  const fetchStudentProfile = async () => {
    try {
      setLoading(true);
      if (user) {
        const { data } = await axios.get(`https://talent-hunt-3.onrender.com/api/student/profile/${user.uid}`);
        setStudent(data);
        
        // Fetch saved competitions
        const savedResponse = await axios.get(`https://talent-hunt-3.onrender.com/api/compapp/me/${user.uid}`);
        setSavedCompetitions(savedResponse.data.map(app => app.competition._id));

        // Fetch user teams - ensure we always get an array
        const teamsResponse = await axios.get(`https://talent-hunt-3.onrender.com/api/teams/user/${user.uid}`);
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [compResponse, teamsResponse, mentorsResponse, sdgResponse, userTeamsResponse] = 
        await Promise.all([
          axios.get('https://talent-hunt-3.onrender.com/api/competitions').catch(() => ({ data: [] })),
          axios.get('https://talent-hunt-3.onrender.com/api/invitations/openings?limit=4').catch(() => ({ data: [] })),
          axios.get('https://talent-hunt-3.onrender.com/api/mentor/mentors?limit=4').catch(() => ({ data: [] })),
          axios.get('https://talent-hunt-3.onrender.com/api/sdgs').catch(() => ({ data: [] })),
          axios.get(`https://talent-hunt-3.onrender.com/api/teams/user/${user.uid}`).catch(() => ({ data: { data: [] } }))
        ]);
  ...
  const toggleSaveCompetition = async (id) => {
    try {
      ...
      if (savedCompetitions.includes(id)) {
        await axios.delete(`https://talent-hunt-3.onrender.com/api/compapp/${id}/save`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setSavedCompetitions(savedCompetitions.filter(compId => compId !== id));
      } else {
        await axios.post(`https://talent-hunt-3.onrender.com/api/compapp/${id}/save`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
                ...
                <div key={comp._id} className="w-full flex-shrink-0 px-2">
                  ...
                  >
                    ...
                    <div className="h-48 bg-gray-700 overflow-hidden relative">
                      <img 
                        src={comp.photo ? `https://talent-hunt-3.onrender.com${comp.photo}` : "https://images.unsplash.com/photo-1466611653911-95081537e5b7"} 
                        alt={comp.name}
