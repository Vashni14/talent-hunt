  ...
  const fetchStudentProfile = async () => {
    try {
      ...
      if (user) {
        ...
        const savedResponse = await axios.get(`https://resurgenet-team-match.up.railway.app//api/compapp/me/${user.uid}`);
        setSavedCompetitions(savedResponse.data.map(app => app.competition._id));

        // Fetch user teams - ensure we always get an array
        const teamsResponse = await axios.get(`https://resurgenet-team-match.up.railway.app//api/teams/user/${user.uid}`);
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [compResponse, teamsResponse, mentorsResponse, sdgResponse, userTeamsResponse] = 
        await Promise.all([
          axios.get('https://resurgenet-team-match.up.railway.app//api/competitions').catch(() => ({ data: [] })),
          axios.get('https://resurgenet-team-match.up.railway.app//api/invitations/openings?limit=4').catch(() => ({ data: [] })),
          axios.get('https://resurgenet-team-match.up.railway.app//api/mentor/mentors?limit=4').catch(() => ({ data: [] })),
          axios.get('https://resurgenet-team-match.up.railway.app//api/sdgs').catch(() => ({ data: [] })),
          axios.get(`https://resurgenet-team-match.up.railway.app//api/teams/user/${user.uid}`).catch(() => ({ data: { data: [] } }))
        ]);
  ...
  useEffect(() => {
    if (student?.profilePicture) {
      setProfileImage(student.profilePicture.startsWith('http') ? 
        ...
        `https://resurgenet-team-match.up.railway.app/${student.profilePicture}`);
    } else {
  ...
  const toggleSaveCompetition = async (id) => {
    try {
      ...
      if (savedCompetitions.includes(id)) {
        await axios.delete(`https://resurgenet-team-match.up.railway.app//api/compapp/${id}/save`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setSavedCompetitions(savedCompetitions.filter(compId => compId !== id));
      } else {
        await axios.post(`https://resurgenet-team-match.up.railway.app//api/compapp/${id}/save`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
                ...
                <div key={comp._id} className="w-full flex-shrink-0 px-2">
                  ...
                  >
                    ...
                    <div className="h-48 bg-gray-700 overflow-hidden relative">
                      <img 
                        src={comp.photo ? `https://resurgenet-team-match.up.railway.app/${comp.photo}` : "https://images.unsplash.com/photo-1466611653911-95081537e5b7"} 
                        alt={comp.name}
